...
import scrapy
import crontab_app.spider.weibo.utils.util as util
from scrapy.exceptions import CloseSpider
from crontab_app.spider.weibo.items import WeiboItem
import logging

logger = logging.getLogger("crontab_app.spider.keyword_seach")
class KeyWordSearchSpider(scrapy.Spider):
    ...
    def parse_by_hour(self, response):
        ...
        page_count = len(response.xpath('//ul[@class="s-scroll"]/li'))
        if is_empty:
            logger.warning('当前页面搜索结果为空, str:%s', str(response.url))
        elif page_count < self.further_threshold:
            # 解析当前页面
            for weibo in self.parse_weibo(response):
                self.check_environment()
