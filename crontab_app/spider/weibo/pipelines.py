...
from hotsearch_analysis.models import HotSearchItem, WeiboItem
from monitor_plan.models import MonitorPlanItem
import logging

logger = logging.getLogger("crontab_app.spider.pipelines")
settings = get_project_settings()
result_location = "/home/li/data/repo/Weibo-Public-Opinion-System/crontap_test/"
...
class DuplicatesPipeline(object):
    ...
    def process_item(self, item, spider):
        if item['item']['bid'] in self.ids_seen:
            raise DropItem("过滤重复微博: %s" % item)
        else:
            self.ids_seen.add(item['item']['bid'])
            return item

class HotSearchPipeline(object):
    def process_item(self, item, spider):
        if item:
            logger.warning("Hotsearch: ", item['title'])
            # 根据热搜标题去除重复热搜（Option：或许存在一条热搜存活时间超过4个小时的情况）
            if not HotSearchItem.objects.filter(title=item['title']):
            # if not HotSearchItem.objects.filter(url=item['url']):
                item.save()
        return item
...
class WeiboPipeline(object):
    def process_item(self, item, spider):
        if item['item']:
            # 去除重复的微博
            try:
                if not WeiboItem.objects.filter(bid=item['item']['bid']):
                    item['item']['hotsearch_id'] = HotSearchItem.objects.get(id=item["hotsearch_id"])
                    item['item'].save()
            except TypeError:
                logger.error("Catch TypeError, item:%s\n hotsearch_item:%s\n", str(item), str(HotSearchItem.objects.get(id=item["hotsearch_id"])))
        return item['item']
...
class KeyWordPipeline(object):
    def process_item(self, item, spider):
        if item['item']:
            # 去除重复的微博
            if not WeiboItem.objects.filter(bid=item['item']['bid']):
                item['item']['monitor_plan_id'] = item['monitor_plan']
                item['item'].save()
        return item['item']

class CommentPipeline(object):
    def process_item(self, item, spider):
        base_dir = 'result' + os.sep + "comment"
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
        file_path = base_dir + os.sep + str(item['weibo_id']) + '.csv'
        if not os.path.isfile(file_path):
            is_first_write = 1
        else:
            is_first_write = 0
...
class UserPipeline(object):
    def process_item(self, item, spider):
        base_dir = 'result' + os.sep + "user"
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
        file_path = base_dir + os.sep + str(item['_id']) + '.csv'
        if not os.path.isfile(file_path):
            is_first_write = 1
        else:
            is_first_write = 0
