...
import scrapy
import celery_tasks.spider.weibo.utils.util as util
# import spider.weibo.utils.util as util
from scrapy.exceptions import CloseSpider
from celery_tasks.spider.weibo.items import WeiboItem
from ..custom_settings import custom_settings_for_keyword_search_spider
# from spider.weibo.items import WeiboItem
import logging 

logger = logging.getLogger("crontab_app.spider.keyword_search")
class KeyWordSearchSpider(scrapy.Spider):
    ...
    def check_settings(self):
        ...
        self.keyword_list = settings.get('KEYWORD_LIST')
        if not isinstance(self.keyword_list, list):
            if not os.path.isabs(self.keyword_list):
                self.keyword_list = os.getcwd() + os.sep + self.keyword_list
            if not os.path.isfile(self.keyword_list):
                sys.exit('不存在%s文件' % self.keyword_list)
            self.keyword_list = util.get_keyword_list(self.keyword_list)
        ...
        self.weibo_type = util.convert_weibo_type(settings.get('WEIBO_TYPE'))
        self.contain_type = util.convert_contain_type(settings.get('CONTAIN_TYPE'))
        self.regions = util.get_regions(settings.get('REGION'))
        ...
        self.further_threshold = settings.get('FURTHER_THRESHOLD', 46)
    ...
    def check_environment(self):
        ...
        if self.pymongo_error:
            logger.error('系统中可能没有安装pymongo库，请先运行 pip install pymongo ，再运行程序')
            raise CloseSpider()
        if self.mongo_error:
            logger.error('系统中可能没有安装或启动MongoDB数据库，请先根据系统环境安装或启动MongoDB，再运行程序')
            raise CloseSpider()
        if self.pymysql_error:
            logger.error('系统中可能没有安装pymysql库，请先运行 pip install pymysql ，再运行程序')
            raise CloseSpider()
        if self.mysql_error:
            logger.error('系统中可能没有安装或正确配置MySQL数据库，请先根据系统环境安装或配置MySQL，再运行程序')
            raise CloseSpider()
    ...
    def parse_by_hour_province(self, response):
        ...
        page_count = len(response.xpath('//ul[@class="s-scroll"]/li'))
        if is_empty:
            logger.warning('当前页面搜索结果为空', str(response.url))
        elif page_count < self.further_threshold:
            # 解析当前页面
            for weibo in self.parse_weibo(response):
                self.check_environment()
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                if retweet_sel and retweet_sel[0].xpath(
                    ...
                    retweet['retweet_id'] = ''
                    yield {"item": retweet, "topic_url": self.settings.get("URL")}
                    weibo['retweet_id'] = retweet['mid']
                yield {"item": weibo, "monitor_plan": self.settings['MONITOR_PLAN']}
