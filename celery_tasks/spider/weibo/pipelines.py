...
logger = logging.getLogger("crontab_app.spider.pipeline")
    ...
    def process_item(self, item, spider):
        if item['item']:
            ...
            if not WeiboItem.objects.filter(bid=item['item']['bid']):
                item['item']['hotsearch_id'] = HotSearchItem.objects.get(url=item['topic_url'])
                item['item'].save()
        return item['item']
...
class KeyWordPipeline(object):
    def process_item(self, item, spider):
        if item['item']:
            logger.debug("crawl weibo from keyword: ", str(item['item']))
            # 去除重复的微博
            if not WeiboItem.objects.filter(bid=item['item']['bid']):
